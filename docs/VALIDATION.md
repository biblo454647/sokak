...
For release previews, run `bash scripts/make_previews.sh docs/qa` after the motion self-test. This optional publishing step needs FFmpeg, muxes the contact audio into the rain and wiper MP4s, copies the snow MP4 and updates the rain/wiper GIFs. The wiper preview warms up the rain for 48 simulated seconds, then exports 300 consecutive frames including collection, clearing and fresh rain. FFmpeg is not needed to run Sokak.
