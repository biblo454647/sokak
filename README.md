...
Sokak is a native macOS menu bar app for a quiet moment beneath rainy glass: drops approach the pane, tap it, spread briefly and leave water behind. Snow drifts at different depths outside a softly frosted window. Keep working through a transparent overlay, or sit with a real street photograph from İstanbul and around the world and soft stereo ambience.
...
## Streets for the weather

![Rain and snow over İstanbul, Paris, Kyoto, Prague and Québec City](docs/world-streets.jpg)

Actual app renderer frames. [Photographers, sources and tile licenses](docs/WORLD-GALLERY-CREDITS.md).

22 original high-resolution photographs from 12 cities, available offline. The library includes 13 wet-street and rainy-night views, eight snowy views and one grey Bosphorus view for mist. Sunny photographs have been retired.

- **Rain:** İstanbul, Paris, Kyoto, Tokyo, Edinburgh, Amsterdam and Bergen.
- **Snow:** İstanbul, Prague, Québec City, Strasbourg, Tallinn and Bern.

Photographs were selected for wet reflections, overcast or night light, snow cover and useful wide-screen framing. Original files are preserved, from 2560 pixels wide up to 7952 × 5304. These are still photographs with Sokak’s animated weather; brightness and sharpness remain adjustable. Authors, source pages and individual licenses are in [Photograph credits](Resources/PHOTO-CREDITS.md).
...
- The library opens with photographs suitable for the selected weather. Rain uses wet streets or overcast views; snow uses real winter photographs. **All streets** shows the complete rainy, snowy and mist collection. Explicitly choosing an incompatible photo turns automatic matching off, so your choice stays put; switch matching back on in Details whenever you want. Import your own JPEG, PNG, HEIC, or TIFF to add a personal view.
...
The curated locations and weather conditions live in `scripts/scene_catalog.json`. Photo regeneration uses the Python standard library: `python3 scripts/fetch_scenes.py`. Audio regeneration additionally requires NumPy and FFmpeg: `python3 scripts/make_audio.py`; use `--only rain` to rebuild just the rain bed. Glass taps are synthesized by the app. Optional release previews use `bash scripts/make_previews.sh docs/qa` after the motion self-test. All photographs and ambience beds are committed; only the first Sparkle dependency fetch needs Internet access for a normal build.
